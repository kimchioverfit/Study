...
선형 변환은 벡터 공간 $ V $에서 $ W $로의 함수 $ T: V \rightarrow W $로서, 다음 두 가지 조건을 만족하는 함수:
...
벡터 $ \vec{x} = \begin{bmatrix} x \\ y \end{bmatrix} $에 대해 다음과 같은 선형 변환 $ T $을 정의한다:
